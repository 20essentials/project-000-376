...
class CircleLetters extends HTMLElement {
  ...
  drawLetters() {
    ...
    let message = 'TALK IS CHEAP, SHOW ME THE CODE - LINUS TORVALDS ';
    ...
    const rotateLetter = () => {
      let rx = 50 * Math.cos(angle) + 110;
      let ry = 50 * Math.sin(angle) + 110;

      for (let i = 0; i < n_letters; i++) {
        let x = 80 * Math.cos(angle) + rx;
        let y = 80 * Math.sin(angle) + ry;
        let deg = (Math.atan2(y - rx, x - ry) * 180) / Math.PI + 90;
